...
def evaluate(sess, x, y, predictions, X_test, Y_test, batch_size = FLAGS.batch_size):
    def f():
...
def evaluate_adv(sess, x, y, predictions, predictions_adv, X_test, Y_test, batch_size = FLAGS.batch_size):
...
def model_train_adv(model, sess, x, y, X_train, Y_train, X_test, Y_test, evaluate_adv1, train_params, batch_size = FLAGS.batch_size, adv = fgsm, eps=0.3):
    predictions = model(x)
    adv_x = adv(x, predictions, eps)
    predictions_adv = model(adv_x)
    model_train(sess, x, y, predictions, X_train, Y_train,
                predictions_adv=predictions_adv, evaluate=evaluate_adv1(sess, x, y, predictions, predictions_adv, X_test, Y_test, batch_size = FLAGS.batch_size),
                args=train_params)
    #SANITY CHECK
