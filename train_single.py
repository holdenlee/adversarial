...
#fgsm
def single_model(adv_f = fgsm2, f=cnn_model):
    m = f()
    ...
    def model(x,y):
        ...
        #print(y)
        predictions = m(x)
        #predictions = tf.Print(predictions, [predictions], "predictions:", summarize=10)
...
def train_single(f, adv_f, X_train, Y_train, X_test, Y_test, label_smooth = 0, batch_size = 100, eval_steps = 600, learning_rate=0.1, epsilon=0.3, print_steps=100, save_steps =1200, train_dir = 'train_single/', filename = 'model.ckpt', summary_steps=1200, max_steps=3600, verbosity=1):
    ...
    addons = [GlobalStep(),
                ...
                Train(lambda gs: tf.train.AdadeltaOptimizer(learning_rate=learning_rate, rho=0.95, epsilon=1e-08), batch_size, train_feed={'epsilon' : epsilon}, loss = 'combined_loss', print_steps=print_steps, train_dir = FLAGS.train_dir),
                Histograms(), #includes gradients, so has to be done after train
                Saver(save_steps = save_steps, checkpoint_path = filename), 
#checkpoint_path = os.path.join(train_dir, filename)),
